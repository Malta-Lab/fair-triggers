...
def hotflip_attack(averaged_grad, embedding_matrix, trigger_token_ids,
    ...
    # print('averaged_grad', averaged_grad.shape)
    embedding_matrix = embedding_matrix.cpu()
    # print('embedding_matrix', embedding_matrix.shape)
    trigger_token_embeds = torch.nn.functional.embedding(torch.LongTensor(trigger_token_ids),
                                                         embedding_matrix).detach().unsqueeze(0)
    averaged_grad = averaged_grad.unsqueeze(0)
    # print('averaged_grad unsqueezed', averaged_grad.shape)
    gradient_dot_embedding_matrix = torch.einsum("bij,kj->bik",
                                                 (averaged_grad, embedding_matrix))
    # print('gradient_dot_embedding_matrix', gradient_dot_embedding_matrix.shape)
    # print('gradient_dot_embedding_matrix', gradient_dot_embedding_matrix)
